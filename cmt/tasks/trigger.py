...
class AddOffline(AddTrigger):
    ...
    def get_new_dataframe(self, input_file, output_file):
        ...
        for yy, zz in itertools.product(range(*self.yy_range), range(*self.zz_range)):
            ...
            # name = "RecoDoubleIsoTau{}er2p1Jet{}".format(yy, zz)
            goodRecoTau = join_root_selection(["abs(Tau_eta) <= 2.1",
                "Tau_pt >= {0}".format(yy)], op="and")
            # df = df.Define(name,
                # "(Tau_pt[{0}].size() >= 2) && "
                # "(Jet_pt[Jet_pt >= {1}].size() >= 1)".format(goodRecoTau, zz))
            # branch_names.append(name)
            name = "DoubleIsoTau{}er2p1Jet{}dR0p5".format(yy, zz)
            df = df.Define(name,
                "(Tau_pt[{0}].size() >= 2) && "
